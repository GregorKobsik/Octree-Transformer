...
class SubstitutionLinearEmbedding(nn.Module):
    def __init__(self, num_vocab, embed_dim, resolution, spatial_dim):
        ...
        """
        super(SubstitutionLinearEmbedding, self).__init__()
        ...
        # embeddings
        self.value_embedding_1 = nn.Embedding(num_vocab + 1, self.conv_dim, padding_idx=0)
        self.depth_embedding_1 = nn.Embedding(tree_depth + 1, self.conv_dim, padding_idx=0)
        self.spatial_embeddings_1 = nn.ModuleList(
            [nn.Embedding(2 * resolution, self.conv_dim, padding_idx=0) for _ in range(spatial_dim)]
        )

        self.value_embedding_2 = nn.Embedding(num_vocab + 1, self.conv_dim, padding_idx=0)
        self.depth_embedding_2 = nn.Embedding(tree_depth + 1, self.conv_dim, padding_idx=0)
        self.spatial_embeddings_2 = nn.ModuleList(
            [nn.Embedding(2 * resolution, self.conv_dim, padding_idx=0) for _ in range(spatial_dim)]
        )

        # convolutions
        self.linear_1 = nn.Linear(self.stride * self.conv_dim, embed_dim)
        self.linear_2 = nn.Linear(self.stride * self.conv_dim, self.conv_dim)
    ...
    def source(self, value, depth, pos):
        ...
        y = self.linear_2(y.reshape(batch_size, -1, self.stride * self.conv_dim))  # [N, T2', C]
        ...
        return self.linear_1(x.reshape(batch_size, -1, self.stride * self.conv_dim))  # [N, T1', E]
